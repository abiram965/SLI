            ...
            while self.is_detecting:
                ...
                if hands:
                    ...
                    if y1 < y2 and x1 < x2:
                        ...
                        # Create white background image for consistency with training
                        aspectRatio = h / w
                        imgWhite = np.ones((imgSize, imgSize, 3), np.uint8) * 255
                        ...
                        if len(self.labels) > index:
                            ...
                            # Update word based on confidence threshold
                            if confidence > 0.7:  # Base confidence threshold
                                if predicted_class == self.last_prediction:
                                    self.word_confidence_count += 1
                                else:
                                    self.word_confidence_count = 1
                                    self.last_prediction = predicted_class
                                
                                # If we've seen the same prediction enough times
                                if self.word_confidence_count >= self.confidence_threshold:
                                    self.current_word = predicted_class
                                    self.word_confidence_count = 0
                                    
                                    # Update UI
                                    self.root.after(0, lambda word=predicted_class: self.update_current_word(word))
